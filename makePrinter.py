...
def makePrinter():
	...
	uf.critical("Failure to import FreeCAD, check your configuration file.", 'Failure to import FreeCAD: '+str(e), gv.level, source)
	...
	if (App.Version()[1]<'14'):
	    uf.warning("Retr3d has not been tested on FreeCAD version .13 and below. You are using version 0."+App.Version()[1]+". If problems ensue, please upgrade to versions .14 or .16.", "Untested FreeCAD Version 0."+App.Version()[1]+" Revision "+App.Version()[2], gv.level, source)
	if (App.Version()[1]=='14'):
	    if not (os.getcwd() == os.path.dirname(os.path.abspath(__file__))):
		uf.info("FreeCAD Version 0."+App.Version()[1]+" Revision "+App.Version()[2], "FreeCAD Version 0."+App.Version()[1]+" Revision "+App.Version()[2], gv.level, source)
	    else:
		uf.critical("Retr3d on Linux is not compatible with FreeCAD version .14 for command line usage. Please upgrade to .16 to continue", "VersionError: FreeCAD Version 0."+App.Version()[1]+" Revision "+App.Version()[2], gv.level, source)
	        raise versionError
	if (App.Version()[1]=='15'):
	    uf.critical("Retr3d on Linux is not compatible with FreeCAD version .15. Please upgrade to .16 or downgrade to .14 to continue.", "VersionError: FreeCAD Version 0."+App.Version()[1]+" Revision "+App.Version()[2], gv.level, source)
	    raise versionError
	if (App.Version()[1]=='16'):
	    try:
		import FreeCADGui 
